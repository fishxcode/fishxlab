...
- 画布项目 JSON：`localForage`，数据库名 `fishxlab`，storeName `app_state`，key 为 `fishxlab:canvas_store`。
- 我的素材 JSON：`localForage`，数据库名 `fishxlab`，storeName `app_state`，key 为 `fishxlab:asset_store`。
- 图片 Blob：单独存到 `localForage` 实例，数据库名 `fishxlab`，storeName `image_files`。
- 视频等媒体 Blob：单独存到 `localForage` 实例，数据库名 `fishxlab`，storeName `media_files`。
